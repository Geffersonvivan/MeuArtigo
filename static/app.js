...
// Modelos REAIS (deviação aprovada): opus-5 / sonnet-5 / haiku-4.5 + Perplexity sonar.
Object.assign(MODEL_COST, {
  'claude-opus-5':    { mult: 5.00, label: 'Opus 5' },
  'claude-sonnet-5':  { mult: 3.00, label: 'Sonnet 5' },
  'claude-haiku-4-5': { mult: 1.00, label: 'Haiku 4.5' },
  'sonar':            { mult: 3.00, label: 'Sonar' }
});
...
// Modelos REAIS por papel (deviação aprovada) — substituem os fictícios no modo real.
if (typeof window !== 'undefined' && window.__DATA__){
  ROLES.redator.defaultModel = 'claude-sonnet-5';
  ROLES.redator.availableModels = ['claude-opus-5','claude-sonnet-5','claude-haiku-4-5'];
  ROLES.pesquisador.defaultModel = 'sonar';
  ROLES.pesquisador.availableModels = ['sonar'];
  ROLES.revisor.defaultModel = 'claude-haiku-4-5';
  ROLES.revisor.availableModels = ['claude-haiku-4-5','claude-sonnet-5'];
}
...
// Dados REAIS do Django (window.__DATA__) sobrescrevem o mock 'a1'.
if (typeof window !== 'undefined' && window.__DATA__ && window.__DATA__.articles) {
  Object.assign(ARTICLES, window.__DATA__.articles);
}
...
// Avisos de margem REAIS (window.__DATA__.notes) substituem o mock.
if (typeof window !== 'undefined' && window.__DATA__ && Array.isArray(window.__DATA__.notes)) {
  MARGIN_NOTES.length = 0;
  window.__DATA__.notes.forEach(function (n) { MARGIN_NOTES.push(n); });
}
...
// UTIL
// -------------------------------------------------------------------
// Aplica artigo atual, status/versão e topbar reais (window.__DATA__).
if (typeof window !== 'undefined' && window.__DATA__) {
  var D = window.__DATA__;
  if (D.currentArticle) state.currentArticle = D.currentArticle;
  if (D.topbar) {
    state.status = D.topbar.statusClass || state.status;
    state.version = D.topbar.version || state.version;
    var _nm = document.getElementById('doc-name'); if (_nm) _nm.textContent = D.topbar.title;
    var _ar = document.querySelector('.doc-title .area'); if (_ar) _ar.textContent = D.topbar.area;
    var _pl = document.getElementById('status-pill');
    if (_pl) { _pl.className = 'status-pill ' + state.status;
      _pl.innerHTML = D.topbar.statusLabel + ' <span class="v" id="status-version">' + state.version + '</span>'; }
  }
  if (D.state) {
    if (typeof D.state.sourcesToDecide === 'number') state.sourcesToDecide = D.state.sourcesToDecide;
    if (typeof D.state.redFlags === 'number') state.redFlags = D.state.redFlags;
  }
  if (D.models) { Object.keys(D.models).forEach(function (k) { state.models[k] = D.models[k]; }); }
}
  ...
  doc.addEventListener('click', (e) => {
    ...
    if (act){
      ...
      if (kind === 'lock'){
        ...
        if (typeof window !== 'undefined' && window.__DATA__) { openComment(p); }
        else { act.classList.add('active'); setTimeout(() => act.classList.remove('active'), 800); }
      }
      return;
    }
...
// --- Reescrita REAL (Redator/Sonnet em streaming) ---
function getCSRF(){ var m = document.cookie.match(/csrftoken=([^;]+)/); return m ? m[1] : ''; }

function gatherInstruction(rewriteEl){
  var chipMap = { encurtar:'Encurte o parágrafo.', expandir:'Desenvolva com mais profundidade.',
    simplificar:'Simplifique a linguagem, mantendo o rigor.', endurecer:'Endureça o tom.',
    fonte:'Aponte onde falta fonte.' };
  var ta = rewriteEl.querySelector('.rewrite-input');
  var chips = Array.prototype.map.call(rewriteEl.querySelectorAll('.chip.selected'),
    function(c){ return chipMap[c.dataset.chip] || c.dataset.chip; });
  var txt = (ta && ta.value.trim()) || '';
  return [txt].concat(chips).filter(Boolean).join(' ') || 'Melhore a clareza e a fluidez.';
}

async function streamRewrite(pid, cur, instrucao, diffEl){
  var colNew = $('.col-new', diffEl);
  var pk = pid.replace(/^p/, '');
  var sugg = '';
  try {
    var modelo = (window.__DATA__ && window.__DATA__.models) ? window.__DATA__.models.redator : null;
    var resp = await fetch('/workspace/paragraph/' + pk + '/rewrite/', {
      method:'POST', headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()},
      body: JSON.stringify({ instrucao: instrucao, model: modelo })
    });
    if (!resp.ok || !resp.body){ colNew.textContent = '⚠️ Falha na reescrita.'; return; }
    ...
    while (true){
      ...
      for (var i=0;i<parts.length;i++){
        var line = parts[i].replace(/^data: /, '').trim(); if (!line) continue;
        var ev; try { ev = JSON.parse(line); } catch(_){ continue; }
        if (ev.delta){ sugg += ev.delta; colNew.textContent = sugg; }
        else if (ev.error){ colNew.textContent = '⚠️ ' + ev.error; return; }
        else if (ev.done && ev.text){ sugg = ev.text; }
      }
    }
  } catch(e){ colNew.textContent = '⚠️ Falha na conexão.'; return; }
  var d = wordDiff(cur, sugg);
  $('.col-cur', diffEl).innerHTML = d.curOut;
  colNew.innerHTML = d.nxtOut;
  state.suggestion = sugg;
  positionMarginNotes();
}
...
function runRewrite(){
  ...
  // Modo REAL: pede a reescrita ao Redator (Sonnet) em streaming.
  if (typeof window !== 'undefined' && window.__DATA__) {
    const instrucao = gatherInstruction(rewriteEl);
    rewriteEl.classList.remove('open');
    diffEl.classList.add('open');
    state.openRewrite = null; state.openDiff = pid; state.suggestion = null;
    $('.col-cur', diffEl).textContent = cur;
    $('.col-new', diffEl).textContent = '…';
    positionMarginNotes();
    diffEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    streamRewrite(pid, cur, instrucao, diffEl);
    return;
  }

  // Modo mock (protótipo)
  const suggestion = REWRITE_SUGGESTIONS[pid] || currentText;
  const { curOut, nxtOut } = wordDiff(cur, suggestion);
...
function acceptSuggestion(){
  ...
  // Modo REAL: persiste o novo texto no parágrafo (grava no banco + disco).
  if (typeof window !== 'undefined' && window.__DATA__) {
    var pk = pid.replace(/^p/, '');
    fetch('/workspace/paragraph/' + pk + '/accept/', {
      method:'POST', headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()},
      body: JSON.stringify({ texto: state.suggestion })
    }).catch(function(){});
  }
...
// Pipeline REAL (window.__DATA__.pipeline) substitui o mock.
if (typeof window !== 'undefined' && window.__DATA__ && window.__DATA__.pipeline) {
  Object.assign(PIPELINE_DATA, window.__DATA__.pipeline);
}

// Custos exibidos ao lado de cada papel — computados a partir do modelo escolhido
function roleAccumulatedCost(roleKey){
  if (typeof window !== 'undefined' && window.__DATA__ && window.__DATA__.roleCosts
      && typeof window.__DATA__.roleCosts[roleKey] === 'number') {
    return window.__DATA__.roleCosts[roleKey];
  }
  const map = {
...
function renderPipeline(){
  ...
  const roleBlock = (roleKey, data) => {
    ...
    const model = (typeof window !== 'undefined' && window.__DATA__ && window.__DATA__.models && window.__DATA__.models[roleKey])
      || state.models[roleKey];
...
function initSidebar(){
  ...
  search.addEventListener('input', () => {
    ...
    // Modo REAL: busca semântica (RAG) no servidor, com debounce.
    if (typeof window !== 'undefined' && window.__DATA__){
      clearTimeout(searchTimer);
      searchTimer = setTimeout(() => {
        fetch('/workspace/search/?q=' + encodeURIComponent(q))
          .then(r => r.json()).then(d => {
            const items = d.results || [];
            results.innerHTML = items.length ? items.map(m => `
              <a class="search-result" href="/workspace/app/${m.articleId}/" style="display:block">
                <div class="title">${escapeHtml(m.article)}</div>
                <div class="snippet">${escapeHtml(m.snippet)}</div>
                <div class="section">similaridade ${m.sim}</div>
              </a>`).join('')
              : `<div style="padding:16px 12px;color:var(--text-faint);font-size:12px">Nenhum resultado para "<em>${escapeHtml(q)}</em>"</div>`;
    ...
    const matches = SEARCH_INDEX.filter(r => r.term.toLowerCase().includes(ql) || r.article.toLowerCase().includes(ql) || r.snippet.toLowerCase().includes(ql));
    results.innerHTML = matches.length ? matches.map(m => `
      <button class="search-result"><div class="title">${m.article}</div><div class="snippet">${m.snippet}</div><div class="section">${m.section}</div></button>`).join('')
      : `<div style="padding:16px 12px;color:var(--text-faint);font-size:12px">Nenhum resultado para "<em>${escapeHtml(q)}</em>"</div>`;
    results.classList.add('open');
  });
...
function hydrateVersionsReal(){
  if (!window.__DATA__) return;
  fetch('/workspace/article/' + window.__DATA__.articleId + '/snapshots/')
    .then(function(r){ return r.json(); }).then(function(d){
    var snaps = d.snapshots || [];
    var compare = document.querySelector('#modal .ver-compare');
    if (snaps.length < 1){
      if (compare) compare.innerHTML = '<div style="padding:48px;color:var(--text-faint);text-align:center">Ainda não há versões salvas. Gere uma versão final para iniciar o histórico.</div>';
      return;
    }
    var sels = document.querySelectorAll('#modal .ver-sel');
    var bodies = document.querySelectorAll('#modal .ver-body');
    var metas = document.querySelectorAll('#modal .ver-meta');
    sels.forEach(function(sel, idx){
      sel.innerHTML = snaps.map(function(s){ return '<option value="' + s.versao + '">v' + s.versao + ' · ' + escapeHtml(s.label) + '</option>'; }).join('');
      sel.selectedIndex = (idx === 0) ? Math.min(1, snaps.length - 1) : 0;
    });
    function bySel(sel){ return snaps.find(function(s){ return String(s.versao) === sel.value; }); }
    function render(){
      var A = bySel(sels[0]), B = bySel(sels[1]); if (!A || !B) return;
      var dd = wordDiff(A.markdown, B.markdown);
      if (bodies[0]) bodies[0].innerHTML = '<pre style="white-space:pre-wrap;font:inherit;margin:0">' + dd.curOut + '</pre>';
      if (bodies[1]) bodies[1].innerHTML = '<pre style="white-space:pre-wrap;font:inherit;margin:0">' + dd.nxtOut + '</pre>';
      if (metas[0]) metas[0].textContent = A.criado; if (metas[1]) metas[1].textContent = B.criado;
    }
...
function openScreen(name){
  ...
  if (name === 'diff' && typeof window !== 'undefined' && window.__DATA__) hydrateVersionsReal();
...
// -------------------------------------------------------------------
function persistStatus(acao){
  if (typeof window === 'undefined' || !window.__DATA__ || !window.__DATA__.articleId) return;
  fetch('/workspace/article/' + window.__DATA__.articleId + '/status/', {
    method:'POST', headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()},
    body: JSON.stringify({ acao: acao })
  }).then(function(r){ return r.json(); }).then(function(d){
    if (d && d.status && window.__DATA__.topbar){
      window.__DATA__.topbar.statusClass = d.status;
      window.__DATA__.topbar.statusLabel = d.statusLabel;
      window.__DATA__.topbar.version = d.version;
    }
  }).catch(function(){});
}
...
function wireModelSettings(){
  ...
  modal.querySelectorAll('.ms-select').forEach(sel => {
    sel.addEventListener('change', () => {
      ...
      if (typeof window !== 'undefined' && window.__DATA__ && window.__DATA__.models) window.__DATA__.models[role] = sel.value;
      renderPipeline();
...
function wireExport(){
  ...
  modal.querySelectorAll('.ex-opt').forEach(b => b.addEventListener('click', () => {
    ...
    if (typeof window !== 'undefined' && window.__DATA__ && window.__DATA__.articleId){
      window.location.href = '/workspace/article/' + window.__DATA__.articleId + '/export/' + fmt + '/';
      showToast(`Exportando ${fmt.toUpperCase()}${state.status !== 'final' ? ' (carimbo RASCUNHO)' : ''}…`);
      closeModal();
      return;
    }
...
function openComment(p){
  var pid = p.dataset.pid, pk = pid.replace(/^p/, '');
  var existing = document.querySelector('.comment-box[data-for="' + pid + '"]');
  if (existing){ existing.remove(); return; }
  var box = document.createElement('div');
  box.className = 'comment-box'; box.setAttribute('data-for', pid);
  box.style.cssText = 'margin:6px -8px 12px; padding:10px 12px; border:1px solid var(--border-strong); border-radius:4px; background:var(--surface);';
  box.innerHTML = '<div class="uppercase-label" style="margin-bottom:8px">Comentários da seção</div>' +
    '<div class="c-list" style="font-size:12.5px"></div>' +
    '<textarea class="c-input" placeholder="Comentar esta parte…" style="width:100%;min-height:34px;margin-top:8px;padding:8px;border:1px solid var(--border);border-radius:3px;font:inherit;font-size:13px"></textarea>' +
    '<div style="margin-top:6px;text-align:right"><button class="btn-primary c-send" style="height:28px;padding:0 12px;font-size:12px">Comentar</button></div>';
  p.after(box);
  function load(){
    fetch('/workspace/paragraph/' + pk + '/comments/').then(function(r){ return r.json(); }).then(function(d){
      box.querySelector('.c-list').innerHTML = (d.comments || []).map(function(c){
        return '<div style="padding:6px 0;border-bottom:1px solid var(--border)">' + escapeHtml(c.texto) +
          ' <span class="faint" style="font-size:11px">· ' + c.criado + '</span></div>';
      }).join('') || '<div class="faint">Sem comentários ainda.</div>';
    });
  }
  load();
  box.querySelector('.c-send').addEventListener('click', function(){
    var t = box.querySelector('.c-input').value.trim(); if (!t) return;
    fetch('/workspace/paragraph/' + pk + '/comment/', { method:'POST',
      headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()}, body: JSON.stringify({ texto: t }) })
      .then(function(r){ return r.json(); }).then(function(){ box.querySelector('.c-input').value=''; load(); });
  });
  box.querySelector('.c-input').focus();
}

function wzField(label){
  var flds = document.querySelectorAll('#modal .fld');
  for (var i=0;i<flds.length;i++){
    var l = flds[i].querySelector('.fld-label');
    if (l && l.textContent.trim().toLowerCase().indexOf(label.toLowerCase())===0) return flds[i].querySelector('.fld-input');
  }
  return null;
}

function criarArtigo(params){
  showToast('Criando artigo e estrutura… (pode levar alguns segundos)');
  fetch('/workspace/article/create/', { method:'POST',
    headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()}, body: JSON.stringify(params) })
    .then(function(r){ return r.json(); })
    .then(function(d){ if (d.url) location.href = d.url; else showToast(d.error || 'Falha ao criar.'); })
    .catch(function(){ showToast('Falha ao criar.'); });
}

function renderOverlaps(list){
  var warn = document.querySelector('#modal .overlap-warn .ow-body');
  var listEl = document.querySelector('#modal .overlap-list');
  var upd = document.querySelector('#modal .modal-foot .actions .btn-primary');
  if (!list.length){
    if (warn) warn.textContent = 'Nenhuma sobreposição relevante encontrada no seu acervo.';
    if (listEl) listEl.innerHTML = '';
    if (upd) upd.style.display = 'none';
    return;
  }
  window.__topOverlap = list[0].articleId;
  if (warn) warn.textContent = 'Encontrei ' + list.length + ' artigo(s) seu(s) com trechos que provavelmente se repetem.';
  if (listEl) listEl.innerHTML = list.map(function(o){
    return '<div class="overlap-item"><div class="ol-head"><div class="ol-title">' + escapeHtml(o.title) +
      '</div><div class="ol-pct">' + o.pct + '% de sobreposição</div></div>' +
      (o.sections && o.sections.length ? '<div class="ol-sections">Seções coincidentes: ' +
        o.sections.map(function(s){ return '<b>"' + escapeHtml(s) + '"</b>'; }).join(', ') + '</div>' : '') + '</div>';
  }).join('');
  if (upd){ upd.style.display=''; upd.removeAttribute('data-close');
    upd.onclick = function(){ closeModal(); location.href = '/workspace/app/' + window.__topOverlap + '/'; }; }
}

function wireWizardStep2(params){
  fetch('/workspace/overlap/', { method:'POST',
    headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()},
    body: JSON.stringify({ assunto: params.assunto, area: params.area }) })
    .then(function(r){ return r.json(); })
    .then(function(d){ renderOverlaps(d.overlaps || []); })
    .catch(function(){ renderOverlaps([]); });
  var btns = document.querySelectorAll('#modal .modal-foot .actions button');
  // [0] Novo mesmo assim, [1] Continuação → criam; [2] Atualizar → navega (em renderOverlaps)
  [0,1].forEach(function(i){ if (btns[i]){ btns[i].removeAttribute('data-close');
    btns[i].addEventListener('click', function(){ criarArtigo(params); }); } });
}
  ...
  nextBtn.addEventListener('click', () => {
    ...
    if (real){
      var params = {
        ...
        area: (wzField('Área')||{}).value || '',
        tese: (wzField('Tese')||{}).value || '',
...
function hydrateSourcesReal(){
  if (!window.__DATA__ || !window.__DATA__.sources) return;
  var srcs = window.__DATA__.sources;
  var tbody = document.querySelector('.src-table tbody');
  if (tbody) tbody.innerHTML = srcs.map(function(s, i){
    return '<tr class="src-row' + (i===0 ? ' selected' : '') + '" data-idx="' + i + '">' +
      '<td>' + escapeHtml(s.title) + '</td><td>' + escapeHtml(s.tipo) + '</td>' +
      '<td><span class="badge ' + s.badgeClass + '">' + escapeHtml(s.badgeLabel) + '</span></td>' +
      '<td>' + (s.citacoes || 0) + '</td></tr>';
  }).join('');
  var counts = { todas: srcs.length, duvidosa:0, 'nao-loc':0, verificada:0 };
  srcs.forEach(function(s){ if (s.badgeClass in counts) counts[s.badgeClass]++; });
  $$('.sf').forEach(function(f){ var sp=f.querySelector('span'); if (sp && f.dataset.filter in counts) sp.textContent = counts[f.dataset.filter]; });

  function showDetail(i){
    var s = srcs[i]; if (!s) return;
    var d = document.querySelector('.src-detail'); if (!d) return;
    var t = d.querySelector('.sd-title'); if (t) t.textContent = s.title;
    var hb = d.querySelector('.sd-head .badge'); if (hb){ hb.className = 'badge ' + s.badgeClass; hb.textContent = s.badgeLabel; }
    var qs = d.querySelectorAll('.sd-quote');
    if (qs[0]) qs[0].textContent = s.afirmacao || '—';
    if (qs[1]) qs[1].textContent = s.trecho || '—';
    var link = d.querySelector('.sd-link'); if (link){ link.textContent = s.url || '(sem link)'; link.href = s.url || '#'; }
    var acts = d.querySelector('.sd-actions');
    if (acts){
      acts.innerHTML =
        '<button class="btn-ghost" data-a="rejeitar">Rejeitar</button>' +
        '<button class="btn-secondary" data-a="aceitar">Aceitar mesmo assim</button>' +
        '<button class="btn-ghost" data-a="verificar">Reverificar</button>' +
        '<button class="btn-primary" data-a="buscar">Buscar fonte melhor</button>';
      acts.querySelectorAll('button').forEach(function(btn){ btn.addEventListener('click', function(){ srcAction(i, btn.dataset.a, btn); }); });
    }
  }
  function srcAction(i, a, btn){
    var s = srcs[i], id = s.id, old = btn.textContent; btn.disabled = true;
    if (a === 'buscar'){
      btn.textContent = 'Buscando…';
      fetch('/workspace/reference/' + id + '/buscar/', { method:'POST', headers:{'X-CSRFToken':getCSRF()} })
        .then(function(r){ return r.json(); }).then(function(d){ btn.disabled=false; btn.textContent=old;
          showToast(d.fonte ? ('Sugestão: ' + d.fonte.titulo) : (d.texto ? d.texto.slice(0,120) : 'Sem resultado')); })
        .catch(function(){ btn.disabled=false; btn.textContent=old; });
      return;
    }
    var url = a === 'verificar' ? '/workspace/reference/' + id + '/verify/' : '/workspace/reference/' + id + '/decide/';
    fetch(url, { method:'POST', headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()},
  ...
  $$('.src-table .src-row').forEach(function(r){
    r.addEventListener('click', function(){
      $$('.src-row').forEach(function(x){ x.classList.remove('selected'); });
      r.classList.add('selected'); showDetail(parseInt(r.dataset.idx, 10) || 0);
    });
  });
  showDetail(0);
}
...
function wireSources(){
  if (typeof window !== 'undefined' && window.__DATA__) { hydrateSourcesReal(); return; }
  $$('.src-row').forEach(r => {
...
// -------------------------------------------------------------------
// VIEW SWITCHER — landing (deslogado) × workspace (logado)
// -------------------------------------------------------------------
function currentView(){ return document.body.dataset.view || 'landing'; }
...
function goToWorkspace(){
  ...
  setAppHeight();
  // Remensuração de linhas/régua/notas — o workspace foi renderizado no boot
  // mas com display:none, então algumas medidas podem ter ficado 0
  requestAnimationFrame(() => {
    $$('.section').forEach(s => updateSectionLines(s));
    syncRuler();
    positionMarginNotes();
...
// LOGIN MODAL
// -------------------------------------------------------------------
let loginBackEl = null;

function openLoginModal(mode = 'signin'){
  closeLoginModal(); // idempotente
  const back = document.createElement('div');
  back.className = 'modal-back open';
  back.id = 'login-back';
  back.innerHTML = `
    <div class="modal login-modal" style="position:relative">
      <button class="login-close" aria-label="Fechar">×</button>
      <div class="login-brand"><span class="lp-brand-mark">¶</span> Meu Artigo</div>
      <div class="login-tabs" role="tablist">
        <button class="login-tab" data-mode="signin">Entrar</button>
        <button class="login-tab" data-mode="signup">Criar conta</button>
      </div>
  ...
  // Fechar por X, click fora, Esc
  back.querySelector('.login-close').addEventListener('click', closeLoginModal);
  back.addEventListener('click', (e) => {
    if (e.target === back) closeLoginModal();
  });
  document.addEventListener('keydown', escCloseLogin);

  // Trocar aba
  back.querySelectorAll('.login-tab').forEach(t => {
...
function escCloseLogin(e){
  if (e.key === 'Escape' && loginBackEl) closeLoginModal();
}
...
function closeLoginModal(){
  if (loginBackEl){
    loginBackEl.remove();
    loginBackEl = null;
    document.removeEventListener('keydown', escCloseLogin);
  }
}

function setLoginMode(mode){
  if (!loginBackEl) return;
  loginBackEl.querySelectorAll('.login-tab').forEach(t => {
    t.classList.toggle('active', t.dataset.mode === mode);
  });
  const form = loginBackEl.querySelector('#login-form');
  if (mode === 'signup'){
    form.innerHTML = `
      <div class="login-field">
        <label for="li-name">Nome</label>
        <input id="li-name" type="text" autocomplete="name" placeholder="Como você assina os artigos"/>
      </div>
      <div class="login-field">
        <label for="li-email">E-mail</label>
        <input id="li-email" type="email" autocomplete="email" placeholder="voce@escritorio.adv.br"/>
      </div>
      <div class="login-field">
        <label for="li-pass">Senha</label>
        <input id="li-pass" type="password" autocomplete="new-password"/>
      </div>
      <button class="login-submit" data-submit>Criar conta</button>
    `;
  } else {
    form.innerHTML = `
      <div class="login-field">
      ...
      <a href="#" class="login-forgot">Esqueci a senha</a>
      <button class="login-submit" data-submit>Entrar</button>
    `;
  }
  form.querySelector('[data-submit]').addEventListener('click', (e) => {
    e.preventDefault();
    goToWorkspace();
  });
...
// -------------------------------------------------------------------
// LANDING — popula recortes estáticos reaproveitando markup do app
// -------------------------------------------------------------------
function populateLandingPreviews(){
  // Preview do workspace: clona o próprio #workspace-view.
  // Como ainda estamos em landing e o workspace tem display:none, é seguro clonar.
  const heroSlot = $('#ws-preview-hero');
  if (heroSlot && !heroSlot.dataset.filled){
    ...
    //   - marcar o p2-1 como .selected
    //   - open o .rewrite[data-for="p2-1"]
    const paras = clone.querySelectorAll('.paragraph');
    paras.forEach(p => p.classList.remove('demo-hover', 'selected'));
    // Encontra o parágrafo com o texto do 9.504
    const target = Array.from(paras).find(p => p.textContent.includes('9.504'));
    if (target){
      target.classList.add('selected');
      // O balão é o próximo elemento .rewrite depois do parágrafo
      let next = target.nextElementSibling;
      while (next && !next.classList.contains('rewrite')) next = next.nextElementSibling;
      if (next){
        next.classList.add('open');
        // marcar chip "simplificar" como selected pra dar textura
        const chip = next.querySelector('[data-chip="simplificar"]');
        if (chip) chip.classList.add('selected');
        // pré-preencher a textarea
        const ta = next.querySelector('.rewrite-input');
        if (ta) ta.value = 'Deixe mais direto, sem juridiquês.';
      }
    }
...
function initLandingActions(){
  ...
  document.querySelectorAll('[data-open-login]').forEach(b => {
    b.addEventListener('click', () => openLoginModal(b.dataset.openLogin));
  });
}
...
// ==================================================================
// Ligação REAL dos painéis estáticos (sidebar/fontes/memória).
// Só roda quando o Django injeta window.__DATA__; senão fica o mock.
// ==================================================================
(function () {
  if (typeof window === 'undefined' || !window.__DATA__) return;
  var D = window.__DATA__;
  function esc(s){ var d = document.createElement('div'); d.textContent = s == null ? '' : String(s); return d.innerHTML; }

  window.__refreshFontes = function(){
    var D2 = window.__DATA__; if (!D2 || !D2.sources) return;
    var fp = document.querySelector('.tab-panel[data-panel="fontes"]');
    if (!fp) return;
    fp.innerHTML = D2.sources.map(function (s) {
      return '<div class="src-item' + (s.badgeClass !== 'verificada' ? ' duvidosa' : '') + '">' +
        '<div class="src-title">' + esc(s.title) + '</div>' +
        '<div class="src-meta"><span class="badge ' + esc(s.badgeClass) + '">' + esc(s.badgeLabel) + '</span> ' + esc(s.tipo) + '</div>' +
        (s.decide ? '<button class="decide" data-open="sources-view">Decidir</button>' : '') + '</div>';
    }).join('') || '<div class="src-item" style="color:var(--text-faint)">Sem fontes.</div>';
    fp.querySelectorAll('[data-open="sources-view"]').forEach(function (b) {
      b.addEventListener('click', function () { openScreen('sources'); });
    });
    var pb = document.querySelector('.pending-badge');
    if (pb) pb.textContent = D2.sources.filter(function (s) { return s.decide; }).length;
  };

  function run(){
    if (D.sidebar) {
      var tree = document.getElementById('tree');
      if (tree) tree.innerHTML = D.sidebar.map(function (g) {
        return '<div class="tree-area"><div class="tree-area-label">' + esc(g.area) + '</div>' +
          g.items.map(function (it) {
            return '<a class="tree-item' + (it.active ? ' active' : '') + '" href="' + esc(it.url) + '">' +
              '<div class="t-title">' + esc(it.title) + '</div>' +
              '<div class="t-meta"><span class="status-dot ' + esc(it.statusClass) + '"></span>' + esc(it.meta) + '</div></a>';
          }).join('') + '</div>';
      }).join('');
    }
    if (D.sources) window.__refreshFontes();
    if (D.memory) {
      var mp = document.querySelector('.tab-panel[data-panel="memoria"]');
      if (mp) mp.innerHTML = D.memory.map(function (m) {
        return '<div class="mem-card"><div class="m-title">' + esc(m.title) + '</div>' +
          '<div class="m-summary">' + esc(m.summary) + '</div>' +
          '<button class="m-ref">Referenciar em vez de repetir</button></div>';
      }).join('') || '<div class="mem-card" style="color:var(--text-faint)">Nada relacionado no acervo.</div>';
    }
    var pb = document.querySelector('.pending-badge');
    if (pb && D.state) pb.textContent = D.state.sourcesToDecide || 0;
  }
